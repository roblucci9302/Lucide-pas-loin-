/**
 * Test des nouveaux profils CEO, Sales, Manager
 * Phase WOW 1 - Jour 2
 */
...
// Test 1: Vérifier que les 7 profils sont disponibles
console.log('📋 Test 1: Disponibilité des profils');
console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
...
expectedProfiles.forEach(profileId => {
    ...
    if (profile) {
        console.log(`  ✅ ${profile.icon} ${profile.name}`);
        console.log(`     ID: ${profile.id}`);
        console.log(`     Couleur: ${profile.color}`);
    } else {
...
newProfiles.forEach(profileId => {
    ...
    // Vérifier que le prompt peut être construit
    try {
        const systemPrompt = getSystemPrompt(profileId, 'Test context');
        console.log(`   ✅ Prompt système généré: ${systemPrompt.length} caractères`);

        // Vérifier que le prompt contient bien "français"
        if (systemPrompt.toLowerCase().includes('français')) {
            console.log(`   ✅ Contrainte de langue FR présente`);
        } else {
            console.log(`   ⚠️  Contrainte de langue FR absente`);
        }
    } catch (error) {
        console.log(`   ❌ Erreur génération prompt: ${error.message}`);
        promptsOK = false;
    }
});
...
profiles.forEach(profile => {
    const workflows = getWorkflowsForProfile(profile.id);
    const workflowIds = getWorkflowIds(profile.id);
...
console.log(`   • Profils totaux: ${profiles.length}`);
